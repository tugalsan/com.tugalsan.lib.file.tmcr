...
import static com.tugalsan.api.file.tmcr.server.code.table.TS_LibRepCodeTableTags.CODE_BEGIN_TABLE;
import static com.tugalsan.api.file.tmcr.server.code.table.TS_LibRepCodeTableTags.CODE_BEGIN_TABLECELL;
import static com.tugalsan.api.file.tmcr.server.code.table.TS_LibRepCodeTableTags.CODE_END_TABLE;
import static com.tugalsan.api.file.tmcr.server.code.table.TS_LibRepCodeTableTags.CODE_END_TABLECELL;
import static com.tugalsan.api.file.tmcr.server.code.table.TS_LibRepCodeTableTags.CODE_END_TABLECELL_180;
import static com.tugalsan.api.file.tmcr.server.code.table.TS_LibRepCodeTableTags.CODE_END_TABLECELL_270;
import static com.tugalsan.api.file.tmcr.server.code.table.TS_LibRepCodeTableTags.CODE_END_TABLECELL_90;
import static com.tugalsan.api.file.tmcr.server.code.table.TS_LibRepCodeTableTags.CODE_TABLECELL_BORDER;
import static com.tugalsan.api.file.tmcr.server.code.table.TS_LibRepCodeTableTags.CODE_TOKEN_NULL;
import com.tugalsan.api.file.tmcr.server.code.text.*;
import java.util.stream.*;

public class TS_LibRepCodeTableWriter {

    final private static TS_Log d = TS_Log.of(TS_LibRepCodeTableWriter.class);
...
//        }
        ...
        for (var c = 0; c < columnSize; c++) {
            sb.append(t.isHeaderBold() ? TS_LibRepCodeFontWriter.SET_FONT_STYLE_BOLD() : TS_LibRepCodeFontWriter.SET_FONT_STYLE_PLAIN());
            sb.append(TS_LibRepCodeFontWriter.SET_FONT_COLOR_BLACK());
            sb.append(TS_LibRepCodeFontWriter.SET_FONT_SIZE(fontsizeHeader));
            sb.append(getMacroCodeTableCellObject(t.getValueAsObject(0, c), sqlCellprefixIsBold));
        }

        sb.append(TS_LibRepCodeFontWriter.SET_FONT_STYLE_PLAIN());
        sb.append(TS_LibRepCodeFontWriter.SET_FONT_COLOR_BLACK());
        sb.append(TS_LibRepCodeFontWriter.SET_FONT_SIZE(fontsizeData));
    ...
    public static String getMacroCodeTable(TS_SQLResultSet rs, int colPercentage[], int fontsizeHeader, int fontsizeData) {
        ...
        sb.append(TS_LibRepCodeFontWriter.SET_FONT_STYLE_BOLD());
        sb.append(TS_LibRepCodeFontWriter.SET_FONT_COLOR_BLACK());
        sb.append(TS_LibRepCodeFontWriter.SET_FONT_SIZE(fontsizeHeader));
    ...
    public static String getMacroCodeTableCellString(String value) {
        ...
        sb.append(TS_LibRepCodeTextWriter.BEGIN_TEXT_LEFT());
        sb.append(TS_LibRepCodeTextWriter.ADD_TEXT(value));
        sb.append(TS_LibRepCodeTextWriter.END_TEXT());
    ...
    public static String getMacroCodeTableCellObject(Object cellValue, boolean rotateHeader_90, Integer rowSpan_orNull, Integer colSpan_orNull, Integer height_orNull, boolean sqlCellprefixIsBold) {
        ...
        if (cellValue != null && cellValue instanceof TS_LibRepCodeTableObj_CellCode) {
            sb.append(((TS_LibRepCodeTableObj_CellCode) cellValue).code);
        ...
        if (cellValue == null) {
            sb.append(TS_LibRepCodeTextWriter.ADD_TEXT(""));
            ...
            sb.append(TS_LibRepCodeTextWriter.ADD_TEXT(tmp0));
            IntStream.range(0, tmp0.length() - tmp1.length()).forEach(i -> {
                sb.append(TS_LibRepCodeTextWriter.ADD_TEXT_SPC());
            });
            if (sqlCellprefixIsBold) {
                sb.append(TS_LibRepCodeFontWriter.SET_FONT_STYLE_PLAIN());
            }
            sb.append(TS_LibRepCodeTextWriter.ADD_TEXT_VAR_FROMSQL(cellValueSQL.tableAndColumnName, cellValueSQL.id));
            sb.append(TS_LibRepCodeTextWriter.ADD_TEXT(cellValueSQL.suffix));
        } else {
            ...
            if (time != null) {
                sb.append(TS_LibRepCodeTextWriter.ADD_TEXT(cellValueText));
            } else {
                if (sqlCellprefixIsBold) {
                    ...
                    if (idx == -1) {
                        ...
                        sb.append(TS_LibRepCodeTextWriter.ADD_TEXT(prefix));
                        if (sqlCellprefixIsBold) {
                            sb.append(TS_LibRepCodeFontWriter.SET_FONT_STYLE_PLAIN());
                        }
                        sb.append(TS_LibRepCodeTextWriter.ADD_TEXT(text));
                    }
                } else {
                    sb.append(TS_LibRepCodeTextWriter.ADD_TEXT(cellValueText));
                }
