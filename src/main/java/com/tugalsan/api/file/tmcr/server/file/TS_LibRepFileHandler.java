...
import com.tugalsan.api.file.tmcr.client.TGS_LibRepFileTypes;

public class TS_LibRepFileHandler extends TS_LibRepFileInterface {

    final private static TS_Log d = TS_Log.of(TS_LibRepFileHandler.class);
    final private static boolean PARALLEL = false; //may cause unexpected exception: java.lang.OutOfMemoryError: Java heap space

    public void rename(Path dirDat, TS_LibRepParser_Globals macroGlobals) {
        d.ci("rename", macroGlobals.prefferedFileNameLabel);
        if (!macroGlobals.prefferedFileNameLabel.isEmpty()) {
            if (TS_LibRepFileInterface.FILENAME_CHAR_SUPPORT_TURKISH) {
            ...
            if (!TS_LibRepFileInterface.FILENAME_CHAR_SUPPORT_SPACE){
    ...
    public TS_LibRepParser_Globals macroGlobals;
    final public List<TS_LibRepFileInterface> files;
    ...
    private TS_LibRepFileHandler(TS_LibRepParser_Globals macroGlobals, TS_LibRepFileInterface... files) {
        super(true, null, null);
        this.macroGlobals = macroGlobals;
        this.files = TGS_StreamUtils.toLst(Arrays.stream(files));
    }

    public static void use(TS_LibRepParser_Globals macroGlobals, TGS_RunnableType1<TS_LibRepFileHandler> fileHandler) {
        var webWidthScalePercent = 68;
        var webFontHightPercent = 60;
        var webHTMLBase64 = false;
        var webHTMBase64 = true;
        var enableTMCR = macroGlobals.requestedFileTypes.contains(TGS_LibRepFileTypes.FILE_TYPE_TMCR());
        var enableHTML = macroGlobals.requestedFileTypes.contains(TGS_LibRepFileTypes.FILE_TYPE_HTML());
        var enableHTM = macroGlobals.requestedFileTypes.contains(TGS_LibRepFileTypes.FILE_TYPE_HTM());
        var enablePDF = macroGlobals.requestedFileTypes.contains(TGS_LibRepFileTypes.FILE_TYPE_PDF());
        var enableXLSX = macroGlobals.requestedFileTypes.contains(TGS_LibRepFileTypes.FILE_TYPE_XLSX());
        var enableDOCX = macroGlobals.requestedFileTypes.contains(TGS_LibRepFileTypes.FILE_TYPE_DOCX());
        var fileNameFullTMCR = macroGlobals.fileNameLabel + TGS_LibRepFileTypes.FILE_TYPE_TMCR();
        var fileNameFullHTML = macroGlobals.fileNameLabel + TGS_LibRepFileTypes.FILE_TYPE_HTML();
        var fileNameFullHTM = macroGlobals.fileNameLabel + TGS_LibRepFileTypes.FILE_TYPE_HTM();
        var fileNameFullPDF = macroGlobals.fileNameLabel + TGS_LibRepFileTypes.FILE_TYPE_PDF();
        var fileNameFullXLSX = macroGlobals.fileNameLabel + TGS_LibRepFileTypes.FILE_TYPE_XLSX();
        var fileNameFullDOCX = macroGlobals.fileNameLabel + TGS_LibRepFileTypes.FILE_TYPE_DOCX();
        var localfileTMCR = TS_LibRepFileInterface.constructPathForLocalFile(macroGlobals.dirDat, fileNameFullTMCR, macroGlobals.username);
        var localfileHTML = TS_LibRepFileInterface.constructPathForLocalFile(macroGlobals.dirDat, fileNameFullHTML, macroGlobals.username);
        var localfileHTM = TS_LibRepFileInterface.constructPathForLocalFile(macroGlobals.dirDat, fileNameFullHTM, macroGlobals.username);
        var localfilePDF = TS_LibRepFileInterface.constructPathForLocalFile(macroGlobals.dirDat, fileNameFullPDF, macroGlobals.username);
        var localfileXLSX = TS_LibRepFileInterface.constructPathForLocalFile(macroGlobals.dirDat, fileNameFullXLSX, macroGlobals.username);
        var localfileDOCX = TS_LibRepFileInterface.constructPathForLocalFile(macroGlobals.dirDat, fileNameFullDOCX, macroGlobals.username);
        var remotefileTMCR = TS_LibRepFileInterface.constructURLForRemoteFile(true, macroGlobals.dirDat, fileNameFullTMCR, macroGlobals.username, macroGlobals.url);
        var remotefileHTML = TS_LibRepFileInterface.constructURLForRemoteFile(false, macroGlobals.dirDat, fileNameFullHTML, macroGlobals.username, macroGlobals.url);
        var remotefileHTM = TS_LibRepFileInterface.constructURLForRemoteFile(true, macroGlobals.dirDat, fileNameFullHTM, macroGlobals.username, macroGlobals.url);
        var remotefilePDF = TS_LibRepFileInterface.constructURLForRemoteFile(true, macroGlobals.dirDat, fileNameFullPDF, macroGlobals.username, macroGlobals.url);
        var remotefileXLSX = TS_LibRepFileInterface.constructURLForRemoteFile(true, macroGlobals.dirDat, fileNameFullXLSX, macroGlobals.username, macroGlobals.url);
        var remotefileDOCX = TS_LibRepFileInterface.constructURLForRemoteFile(true, macroGlobals.dirDat, fileNameFullDOCX, macroGlobals.username, macroGlobals.url);
        TS_LibRepFileTMCR.use(enableTMCR, macroGlobals, localfileTMCR, remotefileTMCR, tmcr -> {
            TS_LibRepFileWeb.use(enableHTML, macroGlobals, localfileHTML, remotefileHTML, webHTMLBase64, webWidthScalePercent, webFontHightPercent, webHTML -> {
                TS_LibRepFileWeb.use(enableHTM, macroGlobals, localfileHTM, remotefileHTM, webHTMBase64, webWidthScalePercent, webFontHightPercent, webHTM -> {
                    TS_LibRepFilePDF.use(enablePDF, macroGlobals, localfilePDF, remotefilePDF, pdf -> {
                        TS_LibRepFileXLSX.use(enableXLSX, macroGlobals, localfileXLSX, remotefileXLSX, xlsx -> {
                            TS_LibRepFileDOCX.use(enableDOCX, macroGlobals, localfileDOCX, remotefileDOCX, docx -> {
                                var instance = new TS_LibRepFileHandler(macroGlobals,
    ...
    private boolean addText2(String mainText) {
        ...
        if (colors.isEmpty()) {
            colors.add(TS_LibRepCodeFontTags.CODE_TOKEN_FONT_COLOR_BLACK());
            colors.add(TS_LibRepCodeFontTags.CODE_TOKEN_FONT_COLOR_RED());
            colors.add(TS_LibRepCodeFontTags.CODE_TOKEN_FONT_COLOR_YELLOW());
            colors.add(TS_LibRepCodeFontTags.CODE_TOKEN_FONT_COLOR_BLUE());
            colors.add(TS_LibRepCodeFontTags.CODE_TOKEN_FONT_COLOR_GREEN());
            colors.add(TS_LibRepCodeFontTags.CODE_TOKEN_FONT_COLOR_PINK());
            colors.add(TS_LibRepCodeFontTags.CODE_TOKEN_FONT_COLOR_ORANGE());
            colors.add(TS_LibRepCodeFontTags.CODE_TOKEN_FONT_COLOR_CYAN());
            colors.add(TS_LibRepCodeFontTags.CODE_TOKEN_FONT_COLOR_DARK_GRAY());
            colors.add(TS_LibRepCodeFontTags.CODE_TOKEN_FONT_COLOR_GRAY());
            colors.add(TS_LibRepCodeFontTags.CODE_TOKEN_FONT_COLOR_LIGHT_GRAY());
            colors.add(TS_LibRepCodeFontTags.CODE_TOKEN_FONT_COLOR_MAGENTA());
        }
        TGS_Tuple1<Boolean> result = new TGS_Tuple1(true);
