package com.tugalsan.api.file.tmcr.server.code.image;
...
public class TS_LibRepCodeImageCompile {

    final private static TS_Log d = TS_Log.of(TS_LibRepCodeImageCompile.class);

    public static int DEFAULT_QUALITY() {
        return 80;
    }

    public static boolean is_INSERT_IMAGE(TS_LibRepParser_Globals macroGlobals) {
        return macroGlobals.macroLine.startsWith(TS_LibRepCodeImageTags.CODE_INSERT_IMAGE());
    }
     ...
     */
    public static TGS_Tuple3<String, Boolean, String> compile_INSERT_IMAGE(TS_LibRepParser_Globals macroGlobals, Path dirDat) {
        var result = d.createFuncBoolean(TS_LibRepCodeImageTags.CODE_INSERT_IMAGE() + "/" + TS_LibRepCodeImageTags.CODE_INSERT_IMAGE_FROMSQL());
        d.ci(result.value0, "INFO: macroLine: " + macroGlobals.macroLineUpperCase);
        d.ci(result.value0, "INFO: dirDat: " + dirDat.toString());

        //WHICH CODE
        boolean fromSQL;
        if (macroGlobals.macroLineUpperCase.startsWith(TS_LibRepCodeImageTags.CODE_INSERT_IMAGE_FROMSQL())) {
            fromSQL = true;
            if (!TS_LibRepParser_Assure.checkTokenSize(macroGlobals, 10)) {
                return d.returnError(result, "ERROR: on code " + TS_LibRepCodeImageTags.CODE_INSERT_IMAGE_FROMSQL() + ", tokensize should be 10");
            }
        } else {//CODE_INSERT_IMAGE
            fromSQL = false;
            if (!TS_LibRepParser_Assure.checkTokenSize(macroGlobals, 8)) {
                return d.returnError(result, "ERROR: on code " + TS_LibRepCodeImageTags.CODE_INSERT_IMAGE() + ", tokensize should be 8");
            }
        }
        ...
        var rotationTAG = macroGlobals.macroLineTokens.get(fromSQL ? 8 : 7);
        var r = TGS_CastUtils.toInteger(rotationTAG);
        if (r == null) {
            if (Objects.equals(rotationTAG, TS_LibRepCodeImageTags.CODE_TOKEN_LANDSCAPE())) {
                rotationIsLandscape = true;
                r = 0;
            } else if (Objects.equals(rotationTAG, TS_LibRepCodeImageTags.CODE_TOKEN_PORTRAIT())) {
        ...
        //GET TEXTWRAP
        boolean textWrap;
        var wrapText = TGS_CharSetCast.toLocaleUpperCase(macroGlobals.macroLineTokens.get(5));
        if (Objects.equals(wrapText, TS_LibRepCodeImageTags.CODE_TOKEN_TEXTWRAP())) {
            textWrap = true;
        } else if (Objects.equals(wrapText, TS_LibRepCodeImageTags.CODE_TOKEN_NULL())) {
            textWrap = false;
        } else {
            return d.returnError(result, "ERROR: code token[5] error! should be either" + TS_LibRepCodeImageTags.CODE_TOKEN_TEXTWRAP() + " or " + TS_LibRepCodeImageTags.CODE_TOKEN_NULL());
        }
        d.ci(result.value0, "INFO: textWrap is : " + textWrap);

        //GET ALLIGN
        int left0_center1_right2;
        var allignText = TGS_CharSetCast.toLocaleUpperCase(macroGlobals.macroLineTokens.get(4));
        if (Objects.equals(allignText, TS_LibRepCodeImageTags.CODE_TOKEN_LEFT())) {
            left0_center1_right2 = 0;
        } else if (Objects.equals(allignText, TS_LibRepCodeImageTags.CODE_TOKEN_CENTER())) {
            left0_center1_right2 = 1;
        } else if (Objects.equals(allignText, TS_LibRepCodeImageTags.CODE_TOKEN_LEFT())) {
            left0_center1_right2 = 2;
        } else {
            return d.returnError(result, "ERROR: code token[4] error! should be either" + TS_LibRepCodeImageTags.CODE_TOKEN_LEFT() + " or " + TS_LibRepCodeImageTags.CODE_TOKEN_CENTER() + " or " + TS_LibRepCodeImageTags.CODE_TOKEN_RIGHT());
        }
        d.ci(result.value0, "INFO: left0_center1_right2 is : " + left0_center1_right2);

        //GET WIDTH
        var w = TGS_CastUtils.toInteger(macroGlobals.macroLineTokens.get(1));
        if (w == null && !TGS_CharSetCast.equalsLocaleIgnoreCase(macroGlobals.macroLineTokens.get(1), TS_LibRepCodeImageTags.CODE_TOKEN_NULL())) {
            return d.returnError(result, "ERROR: code token[1] error! width is : " + w);
        }
        d.ci(result.value0, "INFO: width is : " + w);

        //GET HEIGHT
        var h = TGS_CastUtils.toInteger(macroGlobals.macroLineTokens.get(2));
        if (h == null && !TGS_CharSetCast.equalsLocaleIgnoreCase(macroGlobals.macroLineTokens.get(2), TS_LibRepCodeImageTags.CODE_TOKEN_NULL())) {
        ...
        //GET RESPECT
        boolean respect;
        var respectTXT = TGS_CharSetCast.toLocaleUpperCase(macroGlobals.macroLineTokens.get(3));
        if (Objects.equals(respectTXT, TS_LibRepCodeImageTags.CODE_TOKEN_RESPECT())) {
            respect = true;
        } else if (Objects.equals(respectTXT, TS_LibRepCodeImageTags.CODE_TOKEN_RESPECT())) {
            respect = false;
        } else {
            return d.returnError(result, "ERROR: code token[3] error! should be either" + TS_LibRepCodeImageTags.CODE_TOKEN_RESPECT() + " or " + TS_LibRepCodeImageTags.CODE_TOKEN_NULL());
        }
        ...
        if (fromSQL) {//FROM SQL
            //GET CREATE
            boolean create;
            var createTXT = TGS_CharSetCast.toLocaleUpperCase(macroGlobals.macroLineTokens.get(9));
            if (Objects.equals(createTXT, TS_LibRepCodeImageTags.CODE_TOKEN_CREATE())) {
                create = true;
            } else if (Objects.equals(createTXT, TS_LibRepCodeImageTags.CODE_TOKEN_NULL())) {
                create = false;
            } else {
                return d.returnError(result, "ERROR: code token[9] error! should be either" + TS_LibRepCodeImageTags.CODE_TOKEN_CREATE() + " or " + TS_LibRepCodeImageTags.CODE_TOKEN_NULL());
            }
            ...
            {
                //GET tableAndColname
                var tableAndColname = TS_LibRepParser_Assure.splitTableDotColname(macroGlobals, 6);
                if (tableAndColname == null) {
                    return d.returnError(result, "ERROR: fromSQL.code token[6] error! tableAndColname:" + macroGlobals.macroLineTokens.get(6));
                }
                d.ci(result.value0, "INFO: fromSQL.tableAndColname is : " + TGS_StringUtils.toString(tableAndColname, ","));

                imageTable = TS_LibRepParser_Assure.getTable(macroGlobals, tableAndColname[0]);
                if (imageTable == null) {
                    return d.returnError(result, "ERROR: fromSQL.code token[6] error! imageTable == null: " + macroGlobals.macroLineTokens.get(6));
                }
                if (d.infoEnable) {
                ...
                imgColIdx = TS_LibRepParser_Assure.getColumnIndex(macroGlobals, imageTable, tableAndColname[1]);
                if (imgColIdx == null) {
                    return d.returnError(result, "ERROR: fromSQL.code token[6] error! imgColIdx == null: " + macroGlobals.macroLineTokens.get(6));
                }
            ...
            //GET ID
            Long id;
            if (macroGlobals.macroLineTokens.get(7).equals(TS_LibRepParser_SelectedId.CODE_TOKEN_SELECTED_ID())) {
                id = macroGlobals.selectedId;
                if (id == null) {
                    return d.returnError(result, "ERROR: fromSQL.code token[7] error! SATIR SEÇİLMEDİ HATASI");
                }
            } else {
                id = TGS_CastUtils.toLong(macroGlobals.macroLineTokens.get(7));
                if (id == null) {
            ...
            String fn = null;
            {
                var fns = TS_LibTableFileListUtils.getFileNames_DataIn(macroGlobals.url,
            ...
            var ref = macroGlobals.macroLineTokens.get(6);
            ...
            if (!TS_FileUtils.isExistFile(preImageLoc)) {//FIX: TRY TO UPGRADE OLD REF CODE
                ...
                if (found == null) {
                    ...
                    macroGlobals.macroLineTokens.set(6, ref);
                    preImageLoc = TS_PathUtils.toPathOrError(ref).value0;
            ...
            if (preImageLoc == null) {//FIX: TRY TO DOWNLOAD URL TO TEMP
                ...
                var fileTmp = TS_LibFilePathUtils.datUsrNameTmp(dirDat, macroGlobals.username).resolve(randomStr + filename);
        ...
        //INIT W and H
        {
            ...
            if (macroGlobals.cellHeight == null) {
                d.ci(result.value0, "INFO: #5.1 SKIP macroGlobals.cellHeight == null");
            } else if (macroGlobals.cellHeight >= h) {
                d.ci(result.value0, "INFO: #5.2 SKIP macroGlobals.cellHeight >= h");
            } else {
                d.ci(result.value0, "INFO: #5.3 DETECTED macroGlobals.cellHeight < h");
                var newImageWidth = 1d * macroGlobals.cellHeight * w / h;
                w = (int) Math.round(newImageWidth);
                h = macroGlobals.cellHeight;
            }
