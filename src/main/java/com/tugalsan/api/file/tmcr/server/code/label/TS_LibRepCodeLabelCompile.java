...
import static com.tugalsan.api.file.tmcr.server.code.label.TS_LibRepCodeLabelTags.CODE_TOKEN_EQUALS;
import static com.tugalsan.api.file.tmcr.server.code.label.TS_LibRepCodeLabelTags.CODE_TOKEN_IF_VAL;
import static com.tugalsan.api.file.tmcr.server.code.label.TS_LibRepCodeLabelTags.CODE_TOKEN_NOT_EQUALS;
import static com.tugalsan.api.file.tmcr.server.code.label.TS_LibRepCodeLabelTags.ERROR;
import com.tugalsan.api.file.tmcr.server.code.parser.*;
import com.tugalsan.lib.rql.client.*;
import com.tugalsan.lib.rql.txt.server.*;
import java.util.*;

public class TS_LibRepCodeLabelCompile {

    final private static TS_Log d = TS_Log.of(TS_LibRepCodeLabelCompile.class);

    public static boolean is_SET_LABEL(TS_LibRepParser_Globals macroGlobals) {
        return macroGlobals.macroLineUpperCase.startsWith(CODE_SET_LABEL());
    }

    public static boolean is_SET_LABEL_ON_SEARCH(TS_LibRepParser_Globals macroGlobals) {
    ...
    public static boolean is_SET_LABEL_ON_ERROR(TS_LibRepParser_Globals macroGlobals) {
    ...
    public static boolean is_GOTO_LABEL(TS_LibRepParser_Globals macroGlobals) {
        return macroGlobals.macroLineUpperCase.startsWith(CODE_GOTO_LABEL());
    }

    public static String get_GOTO_LABEL(TS_SQLConnAnchor anchor, TS_LibRepParser_Globals macroGlobals) {
        //FETCH IDX_0 cmd
        ...
        //    0          1       2    3   4            -------------- 5 ---------------------        6       
        if (!TS_LibRepParser_Assure.checkTokenSize(macroGlobals, new int[]{6, 7})) {
            return ERROR();
        }
        ...
        if (macroGlobals.macroLineTokens.size() == 7) {
            ...
            Integer tableColIdx;
            {
                ...
                var tableAndColname = TS_LibRepParser_Assure.splitTableDotColname(macroGlobals, 5);
                ...
                tableColIdx = TS_LibRepParser_Assure.getColumnIndex(macroGlobals, table, tableAndColname[1]);
                if (tableColIdx == null) {
                    return ERROR();
                }
            }
            ...
            Long id;
            {
                ...
                if (ids.equals(TS_LibRepParser_SelectedId.CODE_TOKEN_SELECTED_ID())) {
