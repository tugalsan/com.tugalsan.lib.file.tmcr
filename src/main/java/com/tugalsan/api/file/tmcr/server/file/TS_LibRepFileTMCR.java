...
public class TS_LibRepFileTMCR extends TS_LibRepFileInterface {

    final private static TS_Log d = TS_Log.of(TS_LibRepFileTMCR.class);

    private TS_LibRepParser_Globals macroGlobals;

    private TS_LibRepFileTMCR(boolean enabled, Path localFile, TGS_Url remoteFile) {
        super(enabled, localFile, remoteFile);
    }

    public static void use(boolean enabled, TS_LibRepParser_Globals macroGlobals, Path localFile, TGS_Url remoteFile, TGS_RunnableType1<TS_LibRepFileTMCR> tmcr) {
        var instance = new TS_LibRepFileTMCR(enabled, localFile, remoteFile);
        try {
