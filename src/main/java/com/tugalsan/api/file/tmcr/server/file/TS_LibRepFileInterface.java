...
public abstract class TS_LibRepFileInterface {

    final private static TS_Log d = TS_Log.of(TS_LibRepFileInterface.class);
    ...
    public boolean equals(Object obj) {
        ...
        var other = (TS_LibRepFileInterface) obj;
    ...
    public TS_LibRepFileInterface(boolean enabled, Path localFile, TGS_Url remoteFile) {
    ...
    final public void renameLocalFileName_ifEnabled(Path dirDat, TS_LibRepParser_Globals macroGlobals) {
        if (!isEnabled()) {
            return;
        }
        var localFilePathStr = localFile.toAbsolutePath().toString();
        var type = TGS_LibRepFileTypes.FILE_TYPE_HTML();
        ...
        type = TGS_LibRepFileTypes.FILE_TYPE_HTM();
        if (localFilePathStr.endsWith(type)) {
            renameLocalFileName_do(true, dirDat, macroGlobals.prefferedFileNameLabel + type, macroGlobals.username, macroGlobals.url);
            return;
        }
        type = TGS_LibRepFileTypes.FILE_TYPE_DOCX();
        if (localFilePathStr.endsWith(type)) {
            renameLocalFileName_do(true, dirDat, macroGlobals.prefferedFileNameLabel + type, macroGlobals.username, macroGlobals.url);
            return;
        }
        type = TGS_LibRepFileTypes.FILE_TYPE_XLSX();
        if (localFilePathStr.endsWith(type)) {
            renameLocalFileName_do(true, dirDat, macroGlobals.prefferedFileNameLabel + type, macroGlobals.username, macroGlobals.url);
            return;
        }
        type = TGS_LibRepFileTypes.FILE_TYPE_PDF();
        if (localFilePathStr.endsWith(type)) {
            renameLocalFileName_do(true, dirDat, macroGlobals.prefferedFileNameLabel + type, macroGlobals.username, macroGlobals.url);
            return;
        }
        type = TGS_LibRepFileTypes.FILE_TYPE_TMCR();
        if (localFilePathStr.endsWith(type)) {
            renameLocalFileName_do(true, dirDat, macroGlobals.prefferedFileNameLabel + type, macroGlobals.username, macroGlobals.url);
            return;
        }
        type = TGS_LibRepFileTypes.FILE_TYPE_ZIP();
        if (localFilePathStr.endsWith(type)) {
            renameLocalFileName_do(true, dirDat, macroGlobals.prefferedFileNameLabel + type, macroGlobals.username, macroGlobals.url);
            return;
        }
