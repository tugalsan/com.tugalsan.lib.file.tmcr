...
public class TS_FileTmcrCodeLabelCompile {
    ...
    public static boolean is_SET_LABEL(TS_FileCommonBall fileCommonBall) {
        return fileCommonBall.macroLineUpperCase.startsWith(CODE_SET_LABEL());
    }

    public static boolean is_SET_LABEL_ON_SEARCH(TS_FileCommonBall fileCommonBall) {
        if (TGS_CharSetCast.equalsLocaleIgnoreCase(fileCommonBall.macroLine, CODE_SET_LABEL() + " " + fileCommonBall.doFind_gotoLabel)) {
            d.ci("is_SET_LABEL_ON_SEARCH", CODE_SET_LABEL() + " " + fileCommonBall.doFind_gotoLabel + " found");
            return true;
        } else {
            return false;
        }
    ...
    public static boolean is_SET_LABEL_ON_ERROR(TS_FileCommonBall fileCommonBall) {
        if (TGS_CharSetCast.equalsLocaleIgnoreCase(fileCommonBall.macroLine, CODE_SET_LABEL() + " " + ERROR())) {
    ...
    public static boolean is_GOTO_LABEL(TS_FileCommonBall fileCommonBall) {
        return fileCommonBall.macroLineUpperCase.startsWith(CODE_GOTO_LABEL());
    }

    public static String get_GOTO_LABEL(TS_SQLConnAnchor anchor, TS_FileCommonBall fileCommonBall) {
        //FETCH IDX_0 cmd
        d.ci("get_GOTO_LABEL", "IDX_0", fileCommonBall.macroLineTokens.get(0));
        String doFind_gotoLabel = null;

        //SIZE 2 COMPILE
        if (fileCommonBall.macroLineTokens.size() == 2) {//i = 0; TODO IF NECESSARY
            doFind_gotoLabel = fileCommonBall.macroLineTokens.get(1);
        ...
        //    0          1       2    3   4            -------------- 5 ---------------------        6       
        if (!TS_FileTmcrParser_Assure.checkTokenSize(fileCommonBall, new int[]{6, 7})) {
            return ERROR();
        }

        //FETCH IDX_1 label
        //FETCH IDX_2 ifval
        var label = fileCommonBall.macroLineTokens.get(1);
        var ifVal = fileCommonBall.macroLineTokens.get(2);
        ...
        var val = fileCommonBall.macroLineTokens.get(3);
        var equalsText = fileCommonBall.macroLineTokens.get(4);
        ...
        String data;
        if (fileCommonBall.macroLineTokens.size() == 7) {
            d.ci("get_GOTO_LABEL", "sz7");
            //FETCH IDX_5 tableAndColname
            TGS_LibRqlTbl table;
            Integer tableColIdx;
            {
                d.ci("get_GOTO_LABEL", "sz7", "IDX_5", fileCommonBall.macroLineTokens.get(5));
                var tableAndColname = TS_FileTmcrParser_Assure.splitTableDotColname(fileCommonBall, 5);
                ...
                tableColIdx = TS_FileTmcrParser_Assure.getColumnIndex(fileCommonBall, table, tableAndColname[1]);
                if (tableColIdx == null) {
                    return ERROR();
                }
            }
            ...
            Long id;
            {
                var ids = fileCommonBall.macroLineTokens.get(6);
                ...
                if (ids.equals(TS_FileTmcrParser_SelectedId.CODE_TOKEN_SELECTED_ID())) {
                    ...
                    if (id == null) {
                        d.ce(fileCommonBall.macroLine + ".id requires you select a row from the table!");
                        d.ce(TGS_StringUtils.toString_ln(fileCommonBall.macroLineTokens));
                    ...
                    if (id == null) {
                        d.ce(fileCommonBall.macroLine + ".id should be a number, is it still mapget???");
                        d.ce(TGS_StringUtils.toString_ln(fileCommonBall.macroLineTokens));
                        return ERROR();
                    }
                }
            ...
            //FETCH data
            {
                ...
                if (data == null) {
                    ...
                    data = "ERROR: " + fileCommonBall.macroLine + ".sniffCell() == null";
                }
            }
            d.ci("get_GOTO_LABEL", "sz7", "data", data);
        } else { //6
            d.ci("get_GOTO_LABEL", "sz6");
            data = fileCommonBall.macroLineTokens.get(5);
