...
public class TS_FileTmcrCodePageCompile {
    ...
    public static boolean is_INSERT_PAGE(TS_FileCommonBall fileCommonBall) {
        return fileCommonBall.macroLine.startsWith(CODE_INSERT_PAGE());
    }

    public static TGS_Tuple3<String, Boolean, String> compile_INSERT_PAGE(TS_FileCommonBall fileCommonBall, TS_FileTmcrFileHandler mifHandler) {
        var result = d.createFuncBoolean("compile_INSERT_PAGE");
        if (!TS_FileTmcrParser_Assure.checkTokenSize(fileCommonBall, 7)) {
            return d.returnError(result, "Token size not 7 error @[" + fileCommonBall.macroLine + "]");
        }
        boolean land;
        var landText = TGS_CharSetCast.toLocaleUpperCase(fileCommonBall.macroLineTokens.get(2));
        ...
        if (!mifHandler.createNewPage(pageSizeAX, land, ml, mr, mt, mb)) {
            return d.returnError(result, "Cannot createNewPage->" + "fileCommonBall.mifHandler.createNewPage(pageSizeAX:" + pageSizeAX + ", land, ml" + ml + ", mr" + mr + ", mt" + mt + ", mb" + mb + ")");
        }
        return d.returnTrue(result);
    }

    public static boolean createNewPageDefault(TS_FileCommonBall fileCommonBall, TS_FileTmcrFileHandler mifHandler) {
